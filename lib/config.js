...
var Config = function(adapter, api) { 
        ...
        finish = function(userConfig) {
            ...
            return new Promise(function(resolve, reject){
                ...
                if(userConfig.StepName === "AddCredentials") {
                    ...
                    result = linnCore.UserConfigResponse.error("User config is at an invalid stage");
                }
    ...
    this.testConfig = function(request) {
        return new Promise(function(resolve, reject){
            API.get('products/_1*testproduct1_/stock', function (error, response, body) {

                if (error) {
                    return reject(error);
                }

                if (response.statusCode === 200 && JSON.parse(body).error == "No data found") {
                    resolve(new linnCore.BaseResponse());
                }
                else {
                    reject(new linnCore.BaseResponse(body));
                }
            });
        });
    }

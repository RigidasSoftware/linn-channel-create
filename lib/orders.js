                ...
                Promise.all(promises).then(function(ordersArr) {
                    for(var i = 0; i < ordersArr.length; i++){
                        ...
                        for(var o = 0; o < orders.length; o++){
                            ...
                            if(!newOrder.TaxRate || isNaN(newOrder.TaxRate)){
                                newOrder.TaxRate = 0;
                            }
                            
                            newOrder.PostalServiceCost = parseFloat(or.shipping_total) * (1 + (newOrder.TaxRate / 100));
                            if(!newOrder.PostalServiceCost || isNaN(newOrder.PostalServiceCost)){
                                newOrder.PostalServiceCost = 0;
                            }
        ...
        setStockOptionDetails = function(product) {
            return new Promise(function(resolve, reject) {
                ...
                function set(foundProduct) {
                    ...
                    var title = product.name + (product.options && product.options.length > 0 ? ":" : "");
            ...
            var promise = (resolve, reject) => {
                getOrderProducts(order.ReferenceNumber).then((result) => {
                    ...
                    for(var i = 0; i < result.length; i++) {
                        ...
                        //TEMPORARY REMOVAL UNTIL CREATE CAN PROVIDE THE SKU IN THE ORDER/:ID/PRODUCTS CALL  
                    }
